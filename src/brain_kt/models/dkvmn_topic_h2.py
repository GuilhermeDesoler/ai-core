...
class DKVMNTopicH2Model(nn.Module):
    def __init__(
        self,
        num_h2: int,
        memory_size: int = 20,
        ...
        super().__init__()
        self.h2_emb = nn.Embedding(num_h2, key_dim, padding_idx=0)
        self.next_h2_emb = nn.Embedding(num_h2, key_dim, padding_idx=0)
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        batch_size, seq_len = h2.shape
        device = h2.device
        value_memory = self.init_value_memory.unsqueeze(0).repeat(batch_size, 1, 1).to(device)
        logits_steps = []

        for t in range(seq_len):
            current_query = self.h2_emb(h2[:, t])
            next_query = self.next_h2_emb(nh2[:, t])
