...
RUNS_DIR = PROJECT_ROOT / "artifacts" / "runs_lpkt_phase1"
...
def split_by_user(dataset: list[dict]) -> tuple[list[dict], list[dict], list[dict]]:
...
def main() -> None:
    ...
    dataset = build_next_item_training_sequences(raw, max_seq_len=MAX_SEQ_LEN, stride=STRIDE)

    train, val, test = split_by_user(dataset)

    mappings = build_id_mappings(train)

    train_ds = KTNextItemDataset(train, mappings.question_to_idx, mappings.skill_to_idx, max_seq_len=MAX_SEQ_LEN)
    val_ds = KTNextItemDataset(val, mappings.question_to_idx, mappings.skill_to_idx, max_seq_len=MAX_SEQ_LEN)
    test_ds = KTNextItemDataset(test, mappings.question_to_idx, mappings.skill_to_idx, max_seq_len=MAX_SEQ_LEN)
    ...
    model = LPKTPhase1(len(mappings.question_to_idx), len(mappings.skill_to_idx)).to(device)
