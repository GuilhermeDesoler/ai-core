...
from brain_kt.preprocessing.build_topic_h2_training_sequences import (
    build_h2_topic_training_sequences,
)
...
EPOCHS = 30
PATIENCE = 5  # ← ajustado para consistência


def set_seed(seed=42):
    random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():  # ← adicionado
...
def evaluate(model, loader, device, criterion):
    ...
    return (
        ...
        compute_auc(probs, targets),
        ((probs > 0.5) == targets).float().mean().item(),
    )
...
def main():
    ...
    dataset = build_h2_topic_training_sequences(
        user_sequences, skill_to_h2, max_seq_len=30, stride=25
    )
