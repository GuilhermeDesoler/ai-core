...
from brain_kt.models.lpkt_topic_h3 import LPKTTopicH3Model
...
RUNS_DIR = PROJECT_ROOT / "artifacts" / "runs_lpkt_h3"
...
def main():
    ...
    if not H3_MAP_PATH.exists():
        raise FileNotFoundError("Run build_skill_hierarchy_from_tree.py first")

    with open(H3_MAP_PATH) as f:
        skill_to_h3 = json.load(f)

    dataset = build_h3_topic_training_sequences(user_sequences, skill_to_h3)
    ...
    train_ds = KTTopicH3Dataset(train, h3_map)
    val_ds = KTTopicH3Dataset(val, h3_map)
    test_ds = KTTopicH3Dataset(test, h3_map)
    ...
    model = LPKTTopicH3Model(len(h3_map)).to(device)
